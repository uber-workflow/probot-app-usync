 ...
 */
// TODO: cache this like partner PRs
 ...
 */
export async function getPRCommitList(pullRequest, subDir) {
  return request(
    ...
    if (commits.length && subDir) {
      return Promise.all(
        commits.map(async commit => {
          ...
          if (files.length) {
            let modifiesFilesAboveSubDir, modifiesFilesInSubDir;

            for (const file of files) {
              if (file.filename.startsWith(subDir)) {
                modifiesFilesInSubDir = true;
              } else {
                modifiesFilesAboveSubDir = true;
              }

              if (modifiesFilesAboveSubDir && modifiesFilesInSubDir) break;
            }

            commit.shouldSync = modifiesFilesInSubDir;
            commit.shouldUseGenericMessage =
              modifiesFilesInSubDir && modifiesFilesAboveSubDir;
          }
