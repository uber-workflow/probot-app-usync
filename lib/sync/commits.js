 ...
 * @returns {Promise<void>}
 */
export async function syncPRCommits(primaryPR, secondaryPR) {
  ...
  if (state.hasMismatch) {
    ...
    if (getRelation(sourceRepoName, targetRepoName) === 'parent') {
      copyOpts.source.subPath = getChild(sourceRepoName, targetRepoName).path;
    } else {
      copyOpts.target.subPath = getChild(targetRepoName, sourceRepoName).path;
    }
