...
import {getChildPath} from '../relationships.js';
...
async function getPartnerCommitSha(opts) {
  ...
  const partnerBranch = await request(
  ...
  let partnerCommitSha = partnerBranch.object.sha;
...
export async function copyCommits(opts) {
  ...
  const [sourceCommits, targetCommitTree] = await Promise.all([
    ...
    request('GET /repos/:repoName/commits/:sha', {
      repoName: target.repoName,
      sha: target.commitSha,
    })
      .then(commit =>
        request('GET /repos/:repoName/git/trees/:sha', {
          repoName: target.repoName,
        ...
        prCommits.map(async commit => {
          ...
          if (subPath) {
            ...
            if (fullCommit.files.length) {
              const modifiesSubPath = commitModifiesSubPath(
                fullCommit,
                subPath,
              );
              ...
              commit.shouldUseGenericMessage =
                modifiesSubPath &&
                commitModifiesAboveSubPath(fullCommit, subPath);
            }
          }
...
async function getCommitLists(primaryPR, secondaryPR) {
  ...
  return Promise.all([
    getPRCommitList(
      primaryPR,
      getChildPath(primaryRepoName, secondaryRepoName),
    ),
    getPRCommitList(
      secondaryPR,
      getChildPath(secondaryRepoName, primaryRepoName),
    ),
  ]);
}
 ...
 * @returns {Promise<void>}
 */
export async function resyncCommits(primaryPR, secondaryPR) {
  const primarySubPath = getChildPath(primaryPR.repoName, secondaryPR.repoName);
  const [primaryCommits, primaryPRInfo, secondaryPRInfo] = await Promise.all([
    getPRCommitList(primaryPR, primarySubPath),
    request('GET /repos/:repoName/pulls/:number', primaryPR),
    request('GET /repos/:repoName/pulls/:number', secondaryPR),
  ]);
  ...
  await copyCommits({
    ...
    preserveCommitDate: true,
    source: {
      branch: primaryPRInfo.head.ref,
      commits: primaryCommits,
      lastCommonIndex: -1,
      repoName: primaryPR.repoName,
      subPath: primarySubPath,
    },
    target: {
      baseBranch: secondaryPRInfo.base.ref,
      branch: secondaryPRInfo.head.ref,
      commitSha: await getPartnerRebaseSha({
        commitSha: primaryCommits[0]._.parents[0].sha,
        fallbackSha: secondaryPRInfo.base.sha,
        partnerBranch: secondaryPRInfo.base.ref,
        partnerRepoName: secondaryPR.repoName,
        repoName: primaryPR.repoName,
        subPath: primarySubPath,
      }),
      repoName: secondaryPRInfo.head.repo.full_name,
      subPath: getChildPath(secondaryPR.repoName, primaryPR.repoName),
    },
  });
}
