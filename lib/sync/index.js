  ...
  if (hasRelationship(repoName)) {
    ...
    if (number) {
      ...
      if (primaryPR && secondaryPR) {
        await sequential([
          () => syncMeta(primaryPR, secondaryPR),
          () => syncStatuses(primaryPR, secondaryPR),
          () => syncState(primaryPR, secondaryPR),
          state => state !== 'MERGED' && syncCommits(primaryPR, secondaryPR),
        ]);
      } else {
 ...
 * @returns {Promise<void>}
 */
export async function syncPRStatuses(opts) {
  const number = await getPRNumberFromOpts(opts);

  return callWithPartner(syncStatuses, {
