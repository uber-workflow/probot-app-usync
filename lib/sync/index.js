...
import {generateKeyFromPR, queue, sequential} from '../utils.js';
import {
  copyCommits,
  getPRCommitList,
  resyncCommits,
...
async function createSecondaryPR(primaryPR, secondaryRepoName) {
  ...
  const prLabelNames = (get(primaryPRInfo, 'labels.nodes') || []).map(
    label => label.name,
  );

  if (prLabelNames.includes('disable-sync')) {
    console.log(
      `\`disable-sync\` label found in ${primaryPR.repoName}#${
        primaryPR.number
      }; skipping`,
    );
    return;
  ...
  const secondaryBaseSha = await getRef(
    `{target {oid}}`,
  ...
  if (secondaryBaseSha) {
    await sequential([
      ...
      // copy commits
      async () => {
        const sourceSubPath = getChildPath(
          primaryPR.repoName,
          secondaryRepoName,
        );
        const targetSubPath = getChildPath(
          secondaryRepoName,
          primaryPR.repoName,
        );
      ...
      async res => {
        if (repoRelation === 'parent') {
          return request('POST /repos/:repoName/issues/:number/comments', {
            ...
            data: {
              body:
                `Looks like this PR modified files in the \`${getChildPath(
                  primaryPR.repoName,
                  secondaryRepoName,
 ...
 */
async function getPRLabelNames(primaryPR, secondaryPR) {
  const [primaryLabelNames, secondaryLabelNames] = await Promise.all([
    request('GET /repos/:repoName/issues/:number/labels', primaryPR).then(
      labels => labels.map(label => label.name),
    ),
    request('GET /repos/:repoName/issues/:number/labels', secondaryPR).then(
      labels => labels.map(label => label.name),
    ),
  ]);

  return [...new Set(primaryLabelNames.concat(secondaryLabelNames))];
}

/**
 * Get list of open PRs; only includes one of two partner PRs
 * @returns {Promise<PROptType[]}
 */
export async function getUniqueOpenPRs() {
  const openPRs = await Promise.all(
    getRepoNames().map(repoName =>
      request('GET /repos/:repoName/pulls', {
        repoName,
      }),
    ),
  ).then(flatten);
  ...
  for (const pullRequest of openPRs) {
    ...
    if (isSecondaryBranchName(headBranch)) {
      const {number, repoName} = parseSecondaryBranchName(headBranch);
      primaryPRs.add(`${repoName}#${number}`);
    }
  }

  return openPRs
    .filter(({number, repoName}) => !primaryPRs.has(`${repoName}#${number}`))
    .map(pullRequest => ({
  ...
  if (!result) {
    ...
    if (branchName) {
      ...
      const pullRequests = await request(
        'GET /repos/:repoName/commits/:commitSha/pulls',
        {
          repoName,
          commitSha,
          headers: {
...
export async function syncPR(opts, syncPoints) {
  ...
  if (hasRelationship(repoName)) {
    ...
    if (number) {
      ...
      const queueKey = generateKeyFromPR(pullRequest);

      // first queue operations by pr to ensure actions to a
      // repo are handled in correct order
      await queue(queueKey, async () => {
        const [primaryPR, secondaryPR] = await getPartnerPR(pullRequest);

        if (primaryPR && secondaryPR) {
          const queueKey = `${generateKeyFromPR(primaryPR)}-${generateKeyFromPR(
            secondaryPR,
          )}`;

          // then queue operations by a set of pr partners to ensure
          // any actions between them are also handled in correct order
          await queue(queueKey, async () => {
            const labelNames = await getPRLabelNames(primaryPR, secondaryPR);

            if (labelNames.includes('disable-sync')) {
              console.log(
                `\`disable-sync\` label found in ${primaryPR.repoName}#${
                  primaryPR.number
                }; skipping`,
...
/**
 * Re-applies commits from primary PR to secondary; clean
 * slate if something went wrong syncing some commits
 * (e.g. a bad rebase or merge)
 * @param {PROptType} pullRequest
 * @returns {Promise<void>}
 */
export async function resyncPR(pullRequest) {
  if (hasRelationship(pullRequest.repoName)) {
    await queue(generateKeyFromPR(pullRequest), async () => {
      const [primaryPR, secondaryPR] = await getPartnerPR(pullRequest);

      if (primaryPR && secondaryPR) {
        await resyncCommits(primaryPR, secondaryPR);
      }
    });
  }
}
