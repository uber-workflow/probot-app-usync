...
const PR_PARTNER_CACHE = new Map();
const BOT_SIGNATURE =
 ...
 * @returns {string}
 */
function generateKeyFromPR(pullRequest) {
  return [pullRequest.repoName, pullRequest.number].join('/');
}

/**
 * @param {PROptType} pullRequest
 * @returns {Promise<[PROptType, PROptType] | []>} [primaryPR, secondaryPR]
 */
export async function getPartnerPR(pullRequest) {
  const cacheKey = generateKeyFromPR(pullRequest);

  if (PR_PARTNER_CACHE.has(cacheKey)) {
    return PR_PARTNER_CACHE.get(cacheKey);
  }
  ...
  if (partnerPR) {
    ...
    PR_PARTNER_CACHE.set(cacheKey, result);
    PR_PARTNER_CACHE.set(generateKeyFromPR(partnerPR), result);
    return result;
  } else {
    return [];
  }
 ...
 */
// TODO: if fork without `maintainersCanModify`, post comment requesting to add it; remove comment once they add; also disable any sync until they fix it
// - add commit status to block merges
  ...
  if (hasRelationship(repoName)) {
    ...
    if (number) {
      ...
      if (primaryPR && secondaryPR) {
        await sequential([
          () => syncPRMeta(primaryPR, secondaryPR),
          () => syncPRStatuses(primaryPR, secondaryPR),
          () => syncPRState(primaryPR, secondaryPR),
          state => state !== 'MERGED' && syncPRCommits(primaryPR, secondaryPR),
        ]);
      } else {
 ...
 * @param {PROptType} pullRequest
 */
async function callWithPartner(fn, pullRequest) {
  const [primaryPR, secondaryPR] = await getPartnerPR(pullRequest);

  if (primaryPR && secondaryPR) {
    return fn(primaryPR, secondaryPR);
  }
}
 ...
 * @returns {Promise<void>}
 */
export async function syncPRCommits(pullRequest) {
  return callWithPartner(syncCommits, pullRequest);
}
 ...
 * @returns {Promise<void>}
 */
export async function syncPRMeta(pullRequest) {
  return callWithPartner(syncMeta, pullRequest);
}
 ...
 * @returns {Promise<void>}
 */
export async function syncPRState(pullRequest) {
  return callWithPartner(syncState, pullRequest);
}
 ...
 * @returns {Promise<void>}
 */
export async function syncPRStatuses(pullRequest) {
  return callWithPartner(syncStatuses, pullRequest);
}
