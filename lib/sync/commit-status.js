...
export function isSyncStatusContext(context) {
  return /^probot\/monorepo-sync\/(primary|secondary)-pr-ci/.test(context);
}
 ...
 */
export async function syncPRStatuses(primaryPR, secondaryPR) {
  const [
      ...
      if (filterSyncStatus(primaryStatuses).length) {
        ...
        if (currentState !== groupedState) {
          return request('POST /repos/:repoName/statuses/:sha', {
            ...
            sha: secondarySha,
            data: {
              context: 'probot/monorepo-sync/primary-pr-ci',
              state: groupedState,
            },
          });
        }
