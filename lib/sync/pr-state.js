...
export async function merge(pullRequest, partnerPR) {
  ...
  let commit_message = `meta:sha:${partnerMergeCommitSha}`;
  ...
  await request(`PUT /repos/:repoName/pulls/:number/merge`, {
    ...
    data: {
      commit_message,
      commit_title: `${pullRequestInfo.title} (${pullRequestInfo.url})`,
      merge_method: 'squash',
    },
  });
}
 ...
 */
export async function syncPRState(primaryPR, secondaryPR) {
