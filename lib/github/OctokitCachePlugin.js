  ...
  if (process.env.MONOREPO_SYNC_DISABLE_CACHE) {
    ...
    octokit.hook.wrap('request', async (request, options) => {
      ...
      if (shouldCache && CACHE.has(resolvedUrl)) {
        ...
        if (REQUESTS_PREVENTED % 250 === 0) {
          console.log(
            `REST requests prevented with cache: ${REQUESTS_PREVENTED}`,
          );
        }
      ...
      if (shouldCache) {
        CACHE.set(resolvedUrl, {...response, _fromCache: true});
      }
