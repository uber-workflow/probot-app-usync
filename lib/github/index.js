...
export async function request(url, props = {}) {
  ...
  return authedRequest(url, props, repoName)
    .then(res => {
      if (!res._fromCache) {
        const remainingRateLimit = get(res, 'headers.x-ratelimit-remaining');

        if (remainingRateLimit && parseInt(remainingRateLimit) % 100 === 0) {
          console.log(`remaining REST requests: ${remainingRateLimit}`);
        }
      }
