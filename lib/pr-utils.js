...
async function createParentPR(childPR) {
...
async function createPlaceholderStatus(PR, context) {
...
function genQueueKey(PR, prefix) {
  const {
    number,
    base: {
...
async function getPartnerPR_old(PR, includeClosed) {
...
async function getPartnerPR(PR, includeClosed) {
...
async function getPRFromPushPayload(payload) {
  return getPR(
    {
...
async function getPRFromChecksPayload(payload) {
  return getPR(
    {
...
async function getPRFromStatusPayload(payload) {
...
async function syncPRStatus(fromPR, toPR, context) {
...
async function syncChildRepos(parentPR) {
...
async function syncChildPR(pushPayload, parentPR, childPR) {
...
async function syncParentPR(pushPayload, childPR, parentPR) {
  const childRepoName = childPR.base.repo.full_name;
  const parentRepoName = parentPR.base.repo.full_name;
