...
function setEnv(value) {
  ...
  utils = require('./relationships.js');
}
...
test('getChildPath', () => {
  setEnv('');
  expect(utils.getChildPath('foo', 'foo')).toBe(undefined);
  setEnv('parent > path/:child');
  expect(utils.getChildPath('parent', 'child')).toBe('path');
  expect(utils.getChildPath('child', 'parent')).toBe(undefined);
});
...
test('getRelation', () => {
  setEnv('');
  expect(utils.getRelation('foo', 'bar')).toBe(undefined);
  setEnv('parent-repo > path/:child-repo');
  expect(utils.getRelation('parent-repo', 'child-repo')).toBe('parent');
  expect(utils.getRelation('child-repo', 'parent-repo')).toBe('child');
});
