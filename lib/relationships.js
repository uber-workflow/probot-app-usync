...
const {MONOREPO_SYNC_RELATIONSHIPS, REPO_RELATIONSHIPS} = process.env;

const REPO_RELATIONSHIP_MAP = parseEnv(
  MONOREPO_SYNC_RELATIONSHIPS || REPO_RELATIONSHIPS,
);

if (REPO_RELATIONSHIPS) {
  console.warn(
    '`REPO_RELATIONSHIPS` environment var is deprecated; use `MONOREPO_SYNC_RELATIONSHIPS` instead',
  );
}
