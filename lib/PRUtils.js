...
const {CommitCopier} = require('./CommitCopier.js');
...
module.exports = class PRUtils {
  ...
  async createParentPR(childPR) {
    ...
    const parentRepoName = this.relationshipMap.get(childRepoName).parent;
    ...
    const childSubpath = this.relationshipMap
      .get(parentRepoName)
      .children.find(child => child.name === childRepoName).path;
  ...
  async syncChildRepos(parentPR) {
    ...
    const {children} = this.relationshipMap.get(parentRepoName);
